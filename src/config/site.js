...
export const RESALIB_DEFAULT_URL = "https://www.resalib.fr/";
...
export const sanitizeReservationUrl = (value) => {
  ...
  try {
    ...
    if (
      ...
      !RESALIB_HOSTNAME_PATTERN.test(parsedUrl.hostname)
    ) {
      return "";
    }
...
export const createSiteConfig = (env = import.meta.env ?? process.env) => {
  const configuredReservationUrl = sanitizeReservationUrl(
    env?.VITE_RESALIB_URL ?? "",
  );
  ...
  return {
    ...
    reservationUrl: configuredReservationUrl || RESALIB_DEFAULT_URL,
