  ...
  <main class="min-h-screen overflow-x-hidden bg-cream-50 px-4 py-12 md:py-24">
    <section class="mx-auto max-w-3xl rounded-[2rem] bg-white p-6 shadow-soft-lg md:p-12">
      ...
      <form class="mt-8 space-y-5" @submit.prevent="submit">
        ...
        <label class="flex items-start gap-3 rounded-2xl p-2 -m-2 text-sm text-gray-700 focus-within:ring-2 focus-within:ring-[#C16A46] focus-within:ring-offset-2"><input v-model="consent" type="checkbox" class="mt-1 h-5 w-5 shrink-0 focus-visible:outline-none focus-visible:ring-0" /><span>J’accepte que Maison Loratu utilise ces informations uniquement pour me recontacter au sujet de cette demande, sans autre usage. *</span></label>
        <p v-if="error" class="text-sm text-red-600">{{ error }}</p>
        <button type="submit" class="inline-flex min-h-11 items-center justify-center rounded-full bg-terracotta-500 px-7 py-3 font-semibold text-white hover:bg-terracotta-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C16A46] focus-visible:ring-offset-2">Préparer ma demande</button>
      </form>
